...
#if 0 
	MadDecAlsaBuffer * madDecHandle = (MadDecAlsaBuffer *)alsaPlayHand;
	AlsaHandle alsaHd = madDecHandle->AlsaPlayHandle;
	AlsaContext *context = (AlsaContext *)alsaHd;
	
	snd_pcm_sw_params_set_silence_threshold(context->handle,context->swparams,512);

	FILE * fpcm = fopen("frm.pcm","wb");
	FILE * fp = fopen("cmdPlay.pcm","rb");
	char buff[4096] = "";
	int count = 0;
