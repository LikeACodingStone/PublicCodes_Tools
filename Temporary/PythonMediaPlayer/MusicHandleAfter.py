...
class AddDeleteBySdCard:
    ...
    def GetMusicSumList(self, music_path):
        subMusicList = {}
        country = []
        hardRock = []
        bluesJazz = []
        ...
        with open(music_path, "r", encoding='utf-8') as fp:
            ...
            for line in allLines:
                lineList = line.strip().split("/")
                if lineList[0] == "Country&Original-Sound":
                    country.append(lineList[1:])
                if lineList[0] == "Hard&Grunge&Rocking":
                    hardRock.append(lineList[1:])
                if lineList[0] == "Blues&Reggae&Funky&Jazz":
                    bluesJazz.append(lineList[1:])
                if lineList[0] == "PostPunk&Spikes&Phychedelic":
                    postPunk.append(lineList[1:])
                if lineList[0] == "Chinese":
                    chinese.append(lineList[1:])
                if lineList[0] == "Punk&Ska":
                    punk.append(lineList[1:])
                if lineList[0] == "Metal&QuickShot":
                    metal.append(lineList[1:])
                if lineList[0] == "Classic-Rock&&Popular":
                    popular.append(lineList[1:])
                if lineList[0] == "Solo&Instrumental&FingerStyle":
                    solo.append(lineList[1:])
        subMusicList["Country&Original-Sound"] = country
        subMusicList["Hard&Grunge&Rocking"] = hardRock
        subMusicList["Blues&Reggae&Funky&Jazz"] = bluesJazz
        subMusicList["PostPunk&Spikes&Phychedelic"] = postPunk
        subMusicList["Chinese"] = chinese
        subMusicList["Punk&Ska"] = punk
        subMusicList["Metal&QuickShot"] = metal
        subMusicList["Classic-Rock&&Popular"] = popular
        subMusicList["Solo&Instrumental&FingerStyle"] = solo

        return subMusicList
    
    def extractLocalExist(self, path):
        fileList = []
        for root, dirs, files in os.walk(path):
            for file in files:
                ...
                setFile.append(file)
                setFile.append(filePath)
                fileList.append(setFile)
        return fileList

    def MoveFiles(self, extratList, sumDict, keyWord):
        meidaList = sumDict[keyWord]
        for mp3List in meidaList:
            if len(mp3List) != 1:
                print(mp3List)
            else:
                newPath = "E:\\音乐" + os.sep + keyWord + os.sep + mp3List[0]
                for fileList in extratList:
                    if fileList[0] == mp3List[0]:
                        print(fileList[1], newPath)
                        shutil.move(fileList[1], newPath)

    def HandleSumMusicList(self):
        sumListTxt = GetCurrentFolder() + os.sep + "MusicSumList.txt"
        sumDict = self.GetMusicSumList(sumListTxt)
        # 这里缺一个从 其他移动到MusicSumForHandle的函数
        localPath = "E:\\音乐\\MusicSumForHandle"
        existList = self.extractLocalExist(localPath)
        self.MoveFiles(existList, sumDict, "Country&Original-Sound")
        self.MoveFiles(existList, sumDict, "Hard&Grunge&Rocking")
        self.MoveFiles(existList, sumDict, "Blues&Reggae&Funky&Jazz")
        self.MoveFiles(existList, sumDict, "PostPunk&Spikes&Phychedelic")
        self.MoveFiles(existList, sumDict, "Chinese")
        self.MoveFiles(existList, sumDict, "Punk&Ska")
        self.MoveFiles(existList, sumDict, "Metal&QuickShot")
        self.MoveFiles(existList, sumDict, "Classic-Rock&&Popular")
        self.MoveFiles(existList, sumDict, "Solo&Instrumental&FingerStyle")
